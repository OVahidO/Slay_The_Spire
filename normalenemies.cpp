...
Cultist::Cultist(bool isMultiplayer, QGraphicsItem *parent)
    : Enemy("Cultist", 48, 54, isMultiplayer, parent) { calculateNextIntent(); }
...
JawWorm::JawWorm(bool isMultiplayer, QGraphicsItem *parent)
    : Enemy("Jaw Worm", 42, 46, isMultiplayer, parent) { calculateNextIntent(); }
...
int Louse::randomBiteDamage() const { return 5 + rand() % 3; }
...
bool Slime::shouldSplit() const { return false; }

AcidSlimeS::AcidSlimeS(bool isMultiplayer, QGraphicsItem* parent)
    : Slime("Small Slime", 8, 12, isMultiplayer, parent) { calculateNextIntent(); }
...
AcidSlimeM::AcidSlimeM(bool isMultiplayer, QGraphicsItem* parent)
    : Slime("Medium Slime", 28, 32, isMultiplayer, parent) { calculateNextIntent(); }
...
AcidSlimeL::AcidSlimeL(bool isMultiplayer, QGraphicsItem* parent)
    : Slime("Large Slime", 68, 72, isMultiplayer, parent) { calculateNextIntent(); }
...
bool AcidSlimeL::shouldSplit() const { return !m_hasSplit && m_currentHP <= m_maxHP / 2; }

///////////////////////////////////////////////////////////////////////////////////////////////////////////

Thief::Thief(QString name, int minHP, int maxHP, int mugDamage, bool isMultiplayer, QGraphicsItem* parent)
    : Enemy(name, minHP, maxHP, isMultiplayer, parent), m_mugDamage(mugDamage) { calculateNextIntent(); }

bool Thief::hasEscaped() const { return m_hasEscaped; }

int Thief::stolenGold() const { return m_stolenGold; }
...
BlueSlaver::BlueSlaver(bool isMultiplayer, QGraphicsItem* parent)
    : Enemy("Blue Slaver", 46, 50, isMultiplayer, parent) { calculateNextIntent(); }
...
RedSlaver::RedSlaver(bool isMultiplayer, QGraphicsItem* parent)
    : Enemy("Red Slaver", 46, 50, isMultiplayer, parent) { calculateNextIntent(); }
